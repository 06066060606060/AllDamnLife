...
<div class="max-w-screen-xl mx-auto ">
    <div class="flex items-center justify-center">
        <div class="container">
            <table
                class="w-full flex flex-row flex-no-wrap rounded-lg overflow-hidden sm:shadow-lg my-5 btnmenu">
                ...
                <tbody class="flex-1 sm:flex-none">
                    @forelse ($cards as $card)
                        <tr class="flex flex-col flex-no wrap sm:table-row mb-2 bg-gray-700  sm:mb-0">
                            <td class="border-grey-light border  text-white p-3 h-12">{{ $card->titre }}</td>
                            <td class="border-grey-light border    h-12 truncate"><img
                                    class="w-10 h-10 mx-auto sm:w-14 sm:h-14" src="{{ $card->image }}" alt="">
                            </td>
                            <td
                                class="border-grey-light border   p-3 h-12 text-white  cursor-pointer">
                                {{ $card->prix }}</td>
                                <td class="p-3 text-green-400 cursor-pointer hover:text-green-600  border-grey-light border  h-12"><a
                                    ...
                                    <td class="border-grey-light border  hover:text-blue-900 h-12">@include('parts.updateProduct')
                                    </td>
                                    <td class="border-grey-light border  hover:text-red-600 h-12">@include('parts.deleteProduct')
                                    </td>
                                    <td class="border-grey-light border hover:text-blue-600   h-12"> @include('parts.activeProduct')
                                    </td>
                        </tr>
                    @empty
