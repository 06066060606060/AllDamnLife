<div class="container flex items-center justify-center max-w-screen-xl px-4 mx-auto space-y-4 sm:justify-between">
    <div class="flex flex-row pr-3 space-x-4 sm:space-x-8">
        <div class="flex items-center justify-center flex-shrink-0 ">
            <img src="/img/logo1.png" class="w-12 h-12 mt-8" alt="">
        </div>
        <div class="flex-wrap items-center hidden space-x-4  md:mt-10 sm:mt-6 sm:space-x-8 cacher">


            <a rel="noopener noreferrer" href="#">Contactez-nous</a>

        </div>
    </div>
    <ul class="flex flex-row flex-wrap pl-3 mb-4 space-x-4 md:space-x-8">
        <span>
            <a href="#">Instagram</a>
        </span>
        <span>
            <a href="#">Facebook</a>
        </span>
    </ul>
</div>
<style>
    @media (min-width: 768px) {
        .cacher {
            display: none;
            display: block;
        }
    }
</style>
